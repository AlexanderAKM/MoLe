...
# # Keep this small and interpretable: only max, median, min molecules.
...
# # Actual target values in the same order (max, median, min).
# actual_targets = [
...
#     molecule_labels=["Molecule 1", "Molecule 2", "Molecule 3"], 
#     actual_targets=actual_targets, 
#     target_labels=["maximum", "median", "minimum"]
# )
...
# %% 
# Now for **qm9 dataset**
MODEL_PATH = str(_REPO / "trained_models/train_qm9/chemberta/chemberta_model_final.bin")
...
SCALER_PATH = str(_REPO / "trained_models/train_qm9/chemberta/normalization_scaler.pkl")
TARGET_COLUMN = "dga"
print(DEVICE)
# %%
full_data = pd.read_csv(FULL_PATH)
...
# on the training data
import pickle
with open(_REPO / "results/qm9/ablation/all_results.pkl", "rb") as f:
    qm9_results = pickle.load(f)
    
plot_ablation_metrics(qm9_results, _REPO / "results/qm9", title = "QM9")
...
results = run_regression_lens(tl_encoder, tl_regressor, scaler, min_max_median_molecules, tokenizer)
plot_individual_molecules_regression_lens(results, results_dir=_REPO / "results/qm9/example_regression_lens", molecule_labels = ["Molecule 4", "Molecule 5", "Molecule 6"], y_label = "Gibbs Free Energies of Atomization At 298K", title = "QM9", actual_targets=actual_targets, target_labels=["maximum", "median", "minimum"])
...
group_results = compare_molecule_groups_regression_lens(
    tl_encoder, tl_regressor, scaler, molecule_groups, tokenizer,
    targets=ordered_targets, results_dir=str(_REPO / "results/qm9/regression_lens"), device=DEVICE
)
plot_group_molecules_regression_lens(group_results, results_dir=_REPO / "results/qm9/regression_lens", mean_y_label = "Mean Gibbs Free Energies of Atomization At 298K", var_y_label = "Variance Gibbs Free Energies of Atomization At 298K", title = "QM9")
