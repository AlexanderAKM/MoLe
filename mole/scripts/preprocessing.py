...
def clustering(data, target_column, output_dir, dataset_name='dataset'):
    ...
    number_of_references = 1200 # Increased number of reference molecules
...
def clustering_hce(data, target_column, output_dir, dataset_name='dataset'):
    ...
    number_of_molecules = len(data['smiles'].values)
    number_of_references = 1200 # Increased number of reference molecules
    fingerprint_generator = rdc.rdFingerprintGenerator.GetAtomPairGenerator(fpSize=fingerprint_size) # AtPairGenerator instead of RDKitFPGenerator
