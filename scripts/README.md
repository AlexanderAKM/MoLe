...
Secondly, the data is split in @data_splitting.py. After that, the models are trained in @training.py. In TL_chem.py all of the mechanistic interpretability techniques are done for all of the models. With @evaluate.py one can only evaluate the models if needed without retraining.
